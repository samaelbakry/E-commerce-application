...
export default async function Categories() {
  ...
  const category: CategoryI[] = (await data?.data) ?? [];
          ...
          <div className="space-y-3">
            <Breadcrumb>
              <BreadcrumbList className="font-mono text-[10px] uppercase tracking-widest text-stone-400">
                ...
                <BreadcrumbSeparator />
                <BreadcrumbItem>
                  <BreadcrumbLink href="/brands" className="hover:text-stone-900 transition-colors">
                    Brands
                  </BreadcrumbLink>
                </BreadcrumbItem>
                <BreadcrumbSeparator />
            ...
            <h1 className="text-4xl sm:text-5xl font-light tracking-tight text-stone-900">
              Department <span className="font-serif italic text-emerald-600">Indices</span>
            </h1>
          </div>

          <p className="max-w-xs text-sm text-stone-500 font-light leading-relaxed">
            Browse through unified taxonomic archives partitioned by utility, material essence, and structural intent.
          </p>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-6 w-full mt-12 relative z-10 my-auto">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-x-6 gap-y-12">
          {category.map((cat, idx) => (
            <div key={cat._id} className="group flex flex-col justify-between h-full">
              <Card className="bg-transparent border-none p-0 shadow-none flex flex-col h-full rounded-none">
                
                <Link href={`/categories/${cat._id}`} className="relative block overflow-hidden bg-white border border-stone-200 rounded-xl p-6 transition-all duration-300 hover:border-stone-400 hover:shadow-md">
                ...
                <div className="pt-4 flex items-start justify-between gap-4 border-t border-stone-200/60 mt-3">
                  <Link href={`/categories/${cat._id}`} className="flex-1">
                    <CardTitle className="text-sm font-bold text-stone-900 tracking-wide uppercase transition-colors duration-200 group-hover:text-emerald-600 line-clamp-1">
                      {cat.name}
                    </CardTitle>
                  </Link>
